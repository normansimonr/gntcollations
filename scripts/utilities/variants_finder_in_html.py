...
for filename in os.listdir(htmls_path):
    if filename.endswith(".html"):
        ...
        for tag in soup.find_all(["span", "p"]):
            if "greek-added" in tag.get("class", []) or "greek-omitted" in tag.get(
                "class", []
            ):
        ...
        for i in range(1, len(contiguous_tags)):
            if "greek-added" in contiguous_tags[i - 1].get("class", []):
                data.append(
                    {
                        "Greek-Omitted": contiguous_tags[i].text,
                        "Greek-Added": contiguous_tags[i - 1].text,
                    }
                )
            elif "greek-added" in contiguous_tags[i].get("class", []):
                data.append(
                    {
                        "Greek-Omitted": contiguous_tags[i - 1].text,
                        "Greek-Added": contiguous_tags[i].text,
                    }
                )
        ...
        df["levenshtein_distance"] = df.apply(
            lambda row: levenshtein_distance(row["Greek-Omitted"], row["Greek-Added"]),
            axis=1,
        )
...
dfs["character_difference"] = dfs.apply(
    lambda row: char_diff(row["Greek-Omitted"], row["Greek-Added"]), axis=1
)

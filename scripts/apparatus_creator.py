...
for book_name in ["The Gospel of Matthew"]:  # manuscript_attestation.keys():
    ...
    for chapter in [1]:#manuscript_attestation[book_name].keys():
        ...
        chapter_qmd_string = yaml_section + f"""# {book_name}
...
Generated by Greek New Testament Collations.

Date generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.

More information at [www.gntcollations.com](www.gntcollations.com).
"""         
        ...
        with open(
            f"../apparatus/{book_name.lower().replace(' ','_')}_{chapter}.qmd", "w", encoding="utf-8"
        ) as file:
